...
def play_next():
    ...
    if current_index:
        next_index = (current_index[0] + 1) % listBox.size()
...
def play_prev():
    current_index = listBox.curselection()
    if current_index:
        prev_index = (current_index[0] - 1) % listBox.size()
...
playButton = tk.Button(canvas, image=play_img, bg='black', borderwidth=0, command=lambda: play_music(listBox.get('active')))
...
nextButton = tk.Button(canvas, image=next_img, bg='black', borderwidth=0, command=play_next)
